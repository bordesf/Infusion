...
def build_MLP_mnist():
    ...
    net['d1'] = ll.batch_norm(ll.DenseLayer(net['d0'], num_units=1200, nonlinearity=lasagne.nonlinearities.rectify))
...
def compute_step_train(network, z_T, infuse_rate, mu_target, var_target, coeff_scale_var=1, epsilon=1e-4):
    z_T = theano.gradient.disconnected_grad(z_T)
    # Get the outputs of the network according to z_T
    outputs = ll.get_output([network['mu'], network['var']], z_T, deterministic=False)
    # Get the mean
    mu_model = outputs[0]
    # Get the variance
...
def compute_step_samples(network, z_T, coeff_scale_var=1, epsilon=1e-5):
    outputs = ll.get_output([network['mu'], network['var']], z_T, deterministic=False)
    # Get the mean
    mu_model = outputs[0]
    # Get the variance
...
## Get samples ##
def run_chaine_samples_for(network, num_steps, mu_prior, var_prior, coeff_scale_var):
    # Sample from prior distribution
    z_T_samples = sample_normal(mu_prior, var_prior)
    list_samples_chain = []
    # Run the chain
    for i in range(num_steps):
        z_T_samples, mu_model_samples, var_model_samples = compute_step_samples(network, z_T_samples, coeff_scale_var)
...
val_batch = theano.function(inputs=[mu_prior, var_prior, mu_target, var_target], outputs=[z_T, lower_bound], allow_input_downcast=True, on_unused_input='ignore')
...
for epoch in range(num_epochs):
    ...
    train_batches = 0
    sigma_target = np.zeros((mu_prior.shape)) + 1e-4
...
x_T, samples_chain, mu_pred, var_pred = fn_sample(mu_pred[0:144], var_pred[0:144])
